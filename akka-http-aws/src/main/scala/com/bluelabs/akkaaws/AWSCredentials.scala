...
object AWSCredentials {
  ...
  private def parseAWSCredentialsFile : Option[AWSCredentials] = {
    ...
    if (new java.io.File(file).canRead) {
      ...
      val ac = default.find(_.trim.startsWith("aws_access_key_id")).map(_.split("=")(1).trim)
      val sk = default.find(_.trim.startsWith("aws_secret_access_key")).map(_.split("=")(1).trim)
      ac.flatMap{ac =>
      sk.map{sk =>
  ...
  def default(implicit as:ActorSystem ,mat:ActorMaterializer) : Future[Option[AWSCredentials]] =
    {

    val opt = {
      val access = System.getenv("AWS_ACCESS_KEY_ID")
      val secret = System.getenv("AWS_SECRET_ACCESS_KEY")
      if (access != null && secret != null) Some(BasicCredentials(access,secret)) else None
    }.orElse{
    ...
    if (opt.isDefined) Future.successful(opt)
      ...
      val request1 = HttpRequest(HttpMethods.GET)
        ...
        .withUri(Uri().withHost(address).withScheme("http").withPath(Uri.Path("/latest/meta-data/iam/security-credentials/")))

      Http().singleRequest(request1).flatMap(r => Unmarshal(r.entity).to[String]).flatMap{ line =>

        val request2 = HttpRequest(HttpMethods.GET)
          .withHeaders(Host(address))
          .withUri(Uri().withHost(address).withScheme("http").withPath(Uri.Path("/latest/meta-data/iam/security-credentials/"+line)))

          Http().singleRequest(request2).flatMap(r => Unmarshal(r.entity).to[JsValue]).map{ js =>
